...
function appDataAccess() {
  ...
  db.getConnection(function (err, con) {
      if (err) {
            ...
            console.log('i'+i)
            console.log('appid in data'+request[i].app_id)
            let appuse = appModel.appmod(request[i]);
            console.log('appid in appuse 1 '+appuse.AppId)
          con.query('Select * from app where AppId = ? and Name = ?', [appuse.AppId,appuse.Name], function (err, userResult) {
                 if (err) {
                          logging.LoggingFunction('appInsert',err);
                 ...
                 con.query('update app set Version = ?,Icon = ?,DeviceId = ?,ModifiedDate = ? where AppId = ? and Name = ?', [appuse.Version,appuse.Icon,appuse.DeviceId,appuse.ModifiedDate,appuse.AppId,appuse.Name], function (err, result) {
                        if (err) {
                       ...
                       console.log('request.length'+appuse.length)
                       console.log('inserting  '+appuse.AppId)
