                ...
                con.query('select * from user where PhoneNumber = ? ', [request.phoneNumber], function (err, result) {
                    if (err) {
                        ...
                        if (result.length > 0) {
                            ...
                            let updateInstance = userModel.updateUserReg(request, result);
                            con.query('update user set Name = ?,DeviceId = ?,FcmToken = ?,ModifiedDate = ? where PhoneNumber = ?', [updateInstance.name, updateInstance.device_id, updateInstance.fcm_token, updateInstance.ModifiedDate, request.phoneNumber], function (err, result) {
                                if (err) {
